...
var monthsFullArr = [ 'Tháng Giêng', 'Tháng Hai', 'Tháng Ba', 'Tháng Tư', 'Tháng Năm', 'Tháng Sáu', 'Tháng Bảy', 'Tháng Tám', 'Tháng Chín', 'Tháng Mười', 'Tháng Mười Một', 'Tháng Mười Hai' ];
var monthsShortArr = [ 'Giêng', 'Hai', 'Ba', 'Bốn', 'Năm', 'Sáu', 'Bảy', 'Tám', 'Chín', 'Mười', 'Mười Một', 'Mười Hai' ];
...
$(document).ready(function(){
    ...
    $('select.select2.hide-searchbox').select2({minimumResultsForSearch: Infinity});
    
    //Prevent users from using enter to submit the form
    $("form").not('.allow_enter').find('input,select').bind("keypress", function (e) {
        if (e.keyCode == 13) {
            //e.preventDefault();
            return false;
        }
    ...
    if($('.filter_section ul li').length == 0)
    ...
    if ($('div.note-success').length){
        setTimeout(function(){$('div.note-success').hide('slow');}, 5000);
    }
});
...
function bindNumber(selector, allow_space)
{
    $(selector).keydown(function(event) {
        ...
        if (event.keyCode == 46 || event.keyCode == 8 || event.keyCode == 9 
            ...
            || (typeof allow_space != 'undefined' && allow_space == 1 && event.keyCode == 32)){
                return;
        }else {
...
{
    if (typeof cssStype == 'undefined')
        cssStype = '';
    var now = new Date();
    var rand = now.getTime();
    $(selector).attr('data-rand-id', rand);
    $(selector).after('<span id="'+rand+'" class="money-label" style="'+cssStype+'"></span>');
...
function bindEventForTableFilters(selector)
{
    ...
    if($('#filter_search').length && $('#filter_start_date').length && $('#filter_end_date').length){
        $('#filter_search').after('&nbsp;<a id="filter_search_today" class="btn btn-success"><i class="fa fa-search"></i>&nbsp;Hôm nay</a>');
...
function setLastColumnWidth()
{
    ...
    $('table.dataTable.dt-responsive thead th:last').width(col_width + table_width);
}
...
function getBranchLatLng(){
    if (google)
        return new google.maps.LatLng(branches[processing_branch_id].lat, branches[processing_branch_id].lng);
    return null;
}

function getBranchSaddr(){
    return branches[processing_branch_id].lat + ','+ branches[processing_branch_id].lng;
}
