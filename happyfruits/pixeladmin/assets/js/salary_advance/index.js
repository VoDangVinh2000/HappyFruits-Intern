...
function search(){
    var params = getParams('admin_search_salary_advance');
...
function bindEvents()
{
    ...
    if (!dataTableObj){
        dataTableObj = $('#dataTables-salaryadvanceslist').dataTable({
           "aoColumnDefs": [
    ...
    $('a.delete_item').click(function(e){
        e.preventDefault();
        if (confirm('Bạn có chắc muốn xóa thông tin tạm ứng này?')){
