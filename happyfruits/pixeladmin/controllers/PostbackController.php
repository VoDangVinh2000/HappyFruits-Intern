...
class PostbackController extends BasePostbackController
{
    ...
    function admin_update_user()
    {
        $this->_beginTransaction();
        $user_id = post('user_id');
        $fields = array('username', 'fullname', 'email', 'type_id', 'enabled', 'do_shipping',
        ...
        if($count_all)
        {
            for($i = $start; $i < $start + $length; $i++)
            {
                ...
                $data[] = array(
                    ...
                    '<a target="_blank" href="'.BASE_URL. $URIs['orders'].'/tim/'.$item['customer_id'].'">'.'Chi tiết'.'</a>',
    ...
    {
        $fields = array('name', 'start_dtm', 'end_dtm', 'temporary_close',
	...
	{
		$this->_beginTransaction();
		$fields = array('provider_name', 'provider_type', 'provider_address', 'mobile', 'description', 'lat', 'lng', 'email',
