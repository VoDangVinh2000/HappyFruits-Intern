        ...
        if (isset($latest_update_dtm['product_price_for_delivery']) && $latest_update_dtm['product_price_for_delivery'] != $product_price_modified_dtm) {
            ...
            if ($products) {
                ...
                foreach ($products as $product) {
                    ...
                    if (
                        strtotime($product['modified_dtm']) <= $product_last_modified_dtm &&
    ...
    function save_order()
    {
        ...
        */
        ...
        if ($customer) {
            ...
            //Nếu lúc thanh toán có tài khoản đang đăng nhập thì xử lý save theo cách khác
            if (isset($_SESSION['user_account'])) {
                if ($customer) {
                    ...
                    if (intval($customer_data['mobile']) == 0) {
                        ...
                        $existed_customer = $this->Customers->select_one(array('or' => array("username"
                        => $_SESSION['user_account'][0]['username']), 'deleted' => 0));
                    if ($existed_customer) {
                        ...
                        //Xử lý theo cách chèn customer_id của session vào bảng order
                        $customer_id = $this->Customers->get_customerID($_SESSION['user_account'][0]['username']);
