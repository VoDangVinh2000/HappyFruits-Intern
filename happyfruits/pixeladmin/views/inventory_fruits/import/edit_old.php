<div id="content-wrapper">
    <div id="page-wrapper">
        <?php if (!$id || !$details):?>
            <div class="row">
                <div class="col-lg-12">
                    <h2 class="page-header"><i class="fa fa-warning"></i> ID phiếu nhập trái cây không chính xác!!</h2>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
        <?php else: ?>
            <div class="row">
                <div class="col-lg-12">
                    <h2 class="page-header"><i class="fa fa-download"></i> Phiếu nhập trái cây - Mã <?=getvalue($import_record, 'id')?></h2>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                ...
                <div class="col-lg-12 col-md-12">
                    ...
                    <form id="frmImportInventory" role="form" method="post" class="one-line" action="<?=BASE_URL?>xu-ly">
                        ...
                        <table class="table table-striped table-bordered table-hover" id="dataTables-import">
                            ...
                            foreach ($inventory_items as $item):?>
                                <tr>
                                    ...
                                    <td>
                                        <?=html_select($providers, 'id', 'provider_name', 'disabled class="form-control" name="item_provider[]"', 'Nhà cung cấp', isset($details[$item['id']])?$details[$item['id']]['provider_id']:'')?>
                                    </td>
                                    <td><input style="display: inline;width: 60px;" type="text" class="form-control float" id="item_quantity" disabled name="item_quantity[]" placeholder="SL" value="<?=isset($details[$item['id']])?$details[$item['id']]['quantity']:''?>"/>&nbsp;<?=$item['unit']?></td>
                                    <td><input type="text" class="form-control number inline" id="item_price" disabled name="item_price[]" placeholder="Đơn giá" maxlength="7" value="<?=isset($details[$item['id']])?$details[$item['id']]['price']:0?>"/><span></span></td>
	                    ...
	                    <div class="form-group">
		                    ...
		                    <input type="text" class="form-control number" id="capital" disabled name="capital" value="<?=getvalue($import_record, 'capital')?>" placeholder="Tiền thu" required="" data-error="Vui lòng nhập số tiền thu" data-minlength="6" data-minlength-error="Vui lòng nhập ít nhất 6 số" maxlength="8" />
		                    <div class="help-block with-errors"></div>
	                    </div>
	                    <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" disabled="" id="description" name="description" rows="2" cols="30"><?=getvalue($import_record, 'description')?></textarea>
