...
if (empty($error_msg)):
    ...
    <body>
    <p>Chào bạn,</p>
    <?php if (!empty($is_foody)):?>
        <p><b>Foody</b> có trên hệ thống của <?=get_setting('short_site_name')?> với thông tin sau:</p>
    <?php elseif (!empty($user)):?>
        <p><?=$user['fullname']?> vừa <?=!empty($is_edit)?'sửa':'thêm'?> đơn hàng với thông tin sau:</p>
    <?php else:?>
        <p>Có khách hàng vừa <?=!empty($is_edit)?'sửa đơn hàng':'đặt hàng'?> trên hệ thống trực tuyến của <?=get_setting('short_site_name')?> với thông tin sau:</p>
    <?php endif;?>
    <?php if (empty($is_foody) && $customer):?>
        ...
        <p><b style="font-size: 20px;">Thông tin giao hàng</b><br/>
            ...
            Địa chỉ: <?=get_booking_address($customer)?><?=$customer->district?><?=!empty($customer->distance)?" ({$customer->distance}km)":''?><br />
            Số điện thoại: <?=$customer->mobile?><br />
        </p>
    <?php endif;?>
    <?php
    $des = $description;
    if ($customer && !empty($customer->description))
    {
        if ($des)
            $des .= '<br/>---<br/>';
    ...
    <p>
        ...
        <b>Số điện thoại:</b> <?=getvalue($branch, 'phone_number', '0938.70.70.15 - 0906.70.70.15')?><br />
