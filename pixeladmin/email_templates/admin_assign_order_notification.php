...
if (empty($error_msg)):
    ...
    <body>
    <p>Chào bạn,</p>
    <p>Cửa hàng có đơn hàng <b>đã xác nhận</b> với thông tin như sau:</p>
    <?=get_booker_details($customer) ?>
    <p><b style="font-size: 20px;">Thông tin giao hàng</b><br/>
        Tên người nhận: <?=$customer->fullname?><br/>
        Địa chỉ: <?=$customer->address?>, quận <?=$customer->district?><?=!empty($customer->distance)?" ({$customer->distance}km)":''?><br />
    ...
    <?php endif; ?>
    <p><b style="font-size: 20px;">Thông tin đơn hàng - <?=$order['code']?></b></p>
    ...
    <p>Bạn có thể xem lại và in đơn hàng tại <a href="<?=ROOT_URL?>in/<?=$order['code']?>">đây</a> hoặc sửa lại đơn hàng tại <a href="<?=ROOT_URL?>sua/<?=$order['code']?>">đây</a>.</p>
