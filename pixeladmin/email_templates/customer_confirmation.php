...
if (empty($error_msg)):
    ...
    <body>
    <p>Chào bạn,</p>
    <p>Cảm ơn bạn đã đặt hàng tại <?=get_setting('short_site_name')?>, bạn có thể sửa đơn hàng tại <a href="<?=frontend_url()?>sua-don-hang/<?=$code?>">đây</a> (hoặc <a href="<?=ROOT_URL?>sua/<?=$code?>">URL này</a>).</p>
    <?=get_booker_details($customer) ?>
    <p><b style="font-size: 20px;">Thông tin giao hàng</b><br/>
        <?php if(!empty($order['prebooking_discount'])):?>
        Thời gian giao hàng dự kiến: <?=date('H:i d/m/Y', strtotime($order['delivery_date']))?><br />
        <?php endif;?>
