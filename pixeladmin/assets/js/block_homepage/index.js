...
function search() {
  var params = getParams("admin_search_product_blockhomepage");
...
function bindEvents() {
  ...
  if (!dataTableObj) {
    ...
    bindFilterInput();
    bindEventForTableFilters(
      ".dataTables_filter #filter_category, .dataTables_filter #filter_type"
    );
  }
  ...
  $(
    ".name, .code, .sequence_number, .english_name, .promotion_price, .unit"
  ).change(function () {
