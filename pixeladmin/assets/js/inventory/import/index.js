...
function search(){
    var params = getParams('admin_search_inventory_import');
...
function getParams(action)
{
    ...
    params['is_fruit'] = $('.for_datatable_filter #is_fruit').val();
    return params;
}
...
function bindEvents()
{
    ...
    oLanguage['sEmptyTable'] = 'Không có phiếu nhập kho nào.';
    if (!dataTableObj){
        dataTableObj = $('#dataTables-importlist').dataTable({
           "aoColumnDefs": [
        ...
        bindFilterInput();
        bindEventForTableFilters('.dataTables_filter #filter_member, .dataTables_filter #filter_warehouse, .dataTables_filter #filter_provider');
