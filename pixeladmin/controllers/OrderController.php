...
class OrderController extends BaseController
{
    ...
    function find()
    {
        ...
        $str_explode = explode('-', $keyword);
        $orders = "";
        if ($str_explode[0] == 'sdt') {
            $orders = $this->Orders->get_listorder_mobile($str_explode[1]);
        } else if ($str_explode[0] == 'customer_id') {
            $orders = $this->Orders->get_listorder_customerID($str_explode[1]);
        }
